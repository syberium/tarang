...
{	
	...
	if (field_or_nlin == "field") {
		local_max_abs_div = max(abs(div));

		MPI_Allreduce(&local_max_abs_div, &max_abs_div, 1, MPI_Real, MPI_MAX, MPI_COMM_WORLD);
		
		if ((print_switch) && (max_abs_div > MYEPS2)) {
			// cout << "my_id = "<< global.mpi.my_id<<endl<<"Ax = " << Ax <<endl;
			// cout << "my_id = "<< global.mpi.my_id<<endl<<"Ay = " << Ay <<endl;
			// cout << "my_id = "<< global.mpi.my_id<<endl<<"Az = " << Az <<endl;
			// cout <<"max_abs_div = "<<max_abs_div<<endl;
			if (master)
