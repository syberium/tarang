...
inline Complex mydot_complex(TinyVector<Complex,3> V, TinyVector<Complex,3> W)
{
	return sum(V*W);
}


inline TinyVector<Real,3> mycross(TinyVector<Real,3> V, TinyVector<Real,3> W)
{
	TinyVector<Real,3> VxW;
	VxW = V(1)*W(2) - V(2)*W(1),
		  V(2)*W(0) - V(0)*W(2),
		  V(0)*W(1) - V(1)*W(0);
    return VxW;
...
inline Real mynorm(TinyVector<Real,3> V)
{
	return sqrt( Vsqr( V(0), V(1), V(2) ) );
}

inline Real mynorm(TinyVector<Complex,3> V)
{
	return sqrt( Vsqr(V) );
}

inline TinyVector<Complex,3> mycross(TinyVector<Complex,3> V, TinyVector<Complex,3> W)
{
	TinyVector<Complex,3> VxW;
	VxW = V(1)*W(2) - V(2)*W(1),
		  V(2)*W(0) - V(0)*W(2),
		  V(0)*W(1) - V(1)*W(0);
    return VxW;
