...
Real Universal::Get_total_energy(Array<Complex,3> A)
{
	...
	MPI_Allreduce(&local_energy, &total_energy, 1, MPI_Real, MPI_SUM, MPI_COMM_WORLD);
...
//Not for whole space, for separate part of shells
Real Universal::Get_local_Sn(Array<Complex,3> A, Array<Complex,3> B, Real n, Real start_shell, Real end_shell)
{
	Real Sn = 0.0;
	Real Kmag;	// Kmag = sqrt(Kx^2+Ky^2+Kz^2)
	...
	for (int lx=0; lx<maxlx; lx++)
		for (int ly=0; ly<maxly; ly++)
			for (int lz=0; lz<maxlz; lz++) {
				...
				if ((Kmag >= start_shell) && (Kmag <= end_shell) && (Kmag <= Kmax))
					Sn += Multiplicity_factor(lx,ly,lz)*my_pow(Kmag,n)*real_cprod(A(lx,ly,lz), B(lx,ly,lz));
			}
...
Real Universal::Get_total_Sn(Array<Complex,3> A, Array<Complex,3> B, Real n, Real start_shell, Real end_shell)
{
	Real local_Sn;
	local_Sn = Get_local_Sn(A, B, n, start_shell, end_shell);
...
//--------------------------------------------------------
//Added for forcing energy spectra
void Universal::Compute_local_shell_spectrum_force(
		Array<Complex,3> A,
		int n,
		Array<Real,1> local_Sk,
		Array<Real,1> local_Sk_count,
...
void Universal::Compute_shell_spectrum(
	...
	static Array<Real,1> local_k(Sk.shape());
	
	local_Sk_count = 0.0;
	local_Sk = 0.0;
	local_k = 0.0;
	
	Compute_local_shell_spectrum(A, n, local_Sk, local_Sk_count, local_k);
	...
	MPI_Allreduce(reinterpret_cast<Real*>(local_k.data()), reinterpret_cast<Real*>(k.data()), data_size, MPI_Real, MPI_SUM, MPI_COMM_WORLD);
...
void Universal::Compute_shell_spectrum_force(
		Array<Complex,3> A,
		int n,
		Array<Real,1> Sk,
		Array<Real,1> k
	...
	Compute_local_shell_spectrum_force(A, n, local_Sk, local_Sk_count, local_k);
		...
		for (int ly=0; ly<maxly; ly++)
			for (int lz=0; lz<maxlz; lz++) {
				...
				if (index <= Kmax) {
                    ...
                    if (Ny==1) {
						local_Sk(index) += factor*my_pow(Kmag,n)*real_cprod(A(lx,ly,lz),B(lx,ly,lz))*(Kmag);
                    }
                    else {
						local_Sk(index) += factor*my_pow(Kmag,n)*real_cprod(A(lx,ly,lz),B(lx,ly,lz))*pow2(Kmag);
						local_k(index) += 2*factor * Kmag;
                    }
...
void Universal::Compute_shell_spectrum (
	...
	local_Sk = 0.0;
	local_k = 0.0;
	Compute_local_shell_spectrum(A, B, n, local_Sk, local_Sk_count, local_k);
...
void Universal::Compute_shell_spectrum_force(
	...
	local_Sk = 0.0;
	local_k = 0.0;
	Compute_local_shell_spectrum_force(A, B, n, local_Sk, local_Sk_count, local_k);
