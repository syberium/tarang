...
//********************************************************************************************* 
void Time_advance_incompress::Euler(FluidVF& U,  FluidVF& W, FluidSF&  T, Pressure& P, FORCE& Force)
{
	
	Compute_force_TO_rhs(U,W,T,P,Force);
	
	Single_time_step(U, W, T, 1, 1, 1);
...
void Time_advance_incompress::RK2(FluidVF& U,  FluidVF& W, FluidSF&  T, Pressure& P, FORCE& Force)		
{
	...
	Single_time_step(U, W, T, 0.5, 0.5, 0.5);			// Goto the mid point
	...
	Single_time_step(U, W, T, 1, 0.5, 1);							
...
void Time_advance_incompress::RK4(FluidVF& U,  FluidVF& W, FluidSF& T, Pressure& P, FORCE& Force)	
{
	...
	Compute_force_TO_rhs(U,W,T,P,Force);   
	Single_time_step(U, W, T, 0.5, 0.5, 0.5);				// u1: Goto the mid point

	// tot_Vrhs += (global.time.Tdt/6)*RHS(t)*exp(-nu k^2 b*dt) with b=1.0
	//	RHS is contained in *nlin
	Compute_RK4_parts(U, W, T, tot_Vrhs, tot_Wrhs, tot_Srhs, 1.0, 1.0/6); 
	...
	Single_time_step(U, W, T, 0.5, 0, 0.5);					// u2: Goto the mid point
	
	Compute_RK4_parts(U, W, T, tot_Vrhs, tot_Wrhs, tot_Srhs, 0.5, 1.0/3);
	...
	Single_time_step(U, W, T, 1, 0.5, 1);					// u3: Go to the end point
	...
	Compute_RK4_parts(U, W, T, tot_Vrhs, tot_Wrhs, tot_Srhs, 0, 1.0/6);
