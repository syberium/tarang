...
void EnergyTr::Compute_flux_helical_decomposition(FluidVF &U) {
	flux_U_grad_Uplus_Uplus = 0.0;
	flux_U_grad_Uplus_Uminus = 0.0;
	flux_Uplus_grad_Uplus_Uplus = 0.0;
	flux_Uplus_grad_Uplus_Uminus = 0.0;
	flux_U_grad_Uminus_Uplus = 0.0;
	flux_U_grad_Uminus_Uminus = 0.0;
	flux_Uminus_grad_Uminus_Uplus = 0.0;
	flux_Uminus_grad_Uminus_Uminus = 0.0;
	...
	plus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(U, Giver);
	flux_U_grad_Uplus_Uplus = Prod_by_spheres_nlinV(U, plus);
	flux_U_grad_Uplus_Uminus = Prod_by_spheres_nlinV(U, minus);

	Nlin_incompress::Compute_nlin(plus, Giver);
	flux_Uplus_grad_Uplus_Uplus = Prod_by_spheres_nlinV(plus, plus);
	flux_Uplus_grad_Uplus_Uminus = Prod_by_spheres_nlinV(plus, minus);

	minus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(U, Giver);
	flux_U_grad_Uminus_Uplus = Prod_by_spheres_nlinV(U, plus);
	flux_U_grad_Uminus_Uminus = Prod_by_spheres_nlinV(U, minus);

	Nlin_incompress::Compute_nlin(minus, Giver);
	flux_Uminus_grad_Uminus_Uplus = Prod_by_spheres_nlinV(minus, plus);
	flux_Uminus_grad_Uminus_Uminus = Prod_by_spheres_nlinV(minus, minus);
}

void EnergyTr::Compute_flux_helical_decomposition(FluidVF &U, FluidVF &B) {
	flux_U_grad_Uplus_Uplus = 0.0;
	flux_U_grad_Uplus_Uminus = 0.0;
	flux_Uplus_grad_Uplus_Uplus = 0.0;
	flux_Uplus_grad_Uplus_Uminus = 0.0;
	...
	flux_B_grad_Bplus_Uplus = 0.0;
	flux_B_grad_Bplus_Uminus = 0.0;
	flux_Bplus_grad_Bplus_Uplus = 0.0;
	flux_Bplus_grad_Bplus_Uminus = 0.0;
	flux_B_grad_Bminus_Uplus = 0.0;
	flux_B_grad_Bminus_Uminus = 0.0;
	flux_Bminus_grad_Bminus_Uplus = 0.0;
	flux_Bminus_grad_Bminus_Uminus = 0.0;

	flux_U_grad_Bplus_Bplus = 0.0;
	flux_U_grad_Bplus_Bminus = 0.0;
	flux_Uplus_grad_Bplus_Bplus = 0.0;
	flux_Uplus_grad_Bplus_Bminus = 0.0;
	flux_U_grad_Bminus_Bplus = 0.0;
	flux_U_grad_Bminus_Bminus = 0.0;
	flux_Uminus_grad_Bminus_Bplus = 0.0;
	flux_Uminus_grad_Bminus_Bminus = 0.0;

	flux_B_grad_Uplus_Bplus = 0.0;
	flux_B_grad_Uplus_Bminus = 0.0;
	flux_Bplus_grad_Uplus_Bplus = 0.0;
	flux_Bplus_grad_Uplus_Bminus = 0.0;
	flux_B_grad_Uminus_Bplus = 0.0;
	flux_B_grad_Uminus_Bminus = 0.0;
	flux_Bminus_grad_Uminus_Bplus = 0.0;
	flux_Bminus_grad_Uminus_Bminus = 0.0;


	FluidVF Uplus("Uplus");
	FluidVF Uminus("Uminus");
	FluidVF Bplus("Bplus");
	FluidVF Bminus("Bminus");

	U.Helical_decomposition(Uplus, Uminus);
	B.Helical_decomposition(Bplus, Bminus);

	// U grad U dot U

	Uplus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(U, Giver);
	flux_U_grad_Uplus_Uplus = Prod_by_spheres_nlinV(U, Uplus);
	flux_U_grad_Uplus_Uminus = Prod_by_spheres_nlinV(U, Uminus);

	Nlin_incompress::Compute_nlin(Uplus, Giver);
	flux_Uplus_grad_Uplus_Uplus = Prod_by_spheres_nlinV(Uplus, Uplus);
	flux_Uplus_grad_Uplus_Uminus = Prod_by_spheres_nlinV(Uplus, Uminus);

	Uminus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(U, Giver);
	flux_U_grad_Uminus_Uplus = Prod_by_spheres_nlinV(U, Uplus);
	flux_U_grad_Uminus_Uminus = Prod_by_spheres_nlinV(U, Uminus);

	Nlin_incompress::Compute_nlin(Uminus, Giver);
	flux_Uminus_grad_Uminus_Uplus = Prod_by_spheres_nlinV(Uminus, Uplus);
	flux_Uminus_grad_Uminus_Uminus = Prod_by_spheres_nlinV(Uminus, Uminus);

	//******************************
	// B grad B dot U

	Bplus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(B, Giver);
	flux_B_grad_Bplus_Uplus = Prod_by_spheres_nlinV(B, Uplus);
	flux_B_grad_Bplus_Uminus = Prod_by_spheres_nlinV(B, Uminus);

	Nlin_incompress::Compute_nlin(Bplus, Giver);
	flux_Bplus_grad_Bplus_Uplus = Prod_by_spheres_nlinV(Bplus, Uplus);
	flux_Bplus_grad_Bplus_Uminus = Prod_by_spheres_nlinV(Bplus, Uminus);

	Bminus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(B, Giver);
	flux_B_grad_Bminus_Uplus = Prod_by_spheres_nlinV(B, Uplus);
	flux_B_grad_Bminus_Uminus = Prod_by_spheres_nlinV(B, Uminus);

	Nlin_incompress::Compute_nlin(Bminus, Giver);
	flux_Bminus_grad_Bminus_Uplus = Prod_by_spheres_nlinV(Bminus, Uplus);
	flux_Bminus_grad_Bminus_Uminus = Prod_by_spheres_nlinV(Bminus, Uminus);
	...
	Nlin_incompress::Compute_nlin(U, Giver);
	flux_U_grad_Bplus_Bplus = Prod_by_spheres_nlinV(U, Bplus);
	flux_U_grad_Bplus_Bminus = Prod_by_spheres_nlinV(U, Bminus);

	Nlin_incompress::Compute_nlin(Uplus, Giver);
	flux_Uplus_grad_Bplus_Bplus = Prod_by_spheres_nlinV(Uplus, Bplus);
	flux_Uplus_grad_Bplus_Bminus = Prod_by_spheres_nlinV(Uplus, Bminus);

	Bminus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(U, Giver);
	flux_U_grad_Bminus_Bplus = Prod_by_spheres_nlinV(U, Bplus);
	flux_U_grad_Bminus_Bminus = Prod_by_spheres_nlinV(U, Bminus);

	Nlin_incompress::Compute_nlin(Uminus, Giver);
	flux_Uminus_grad_Bminus_Bplus = Prod_by_spheres_nlinV(Uminus, Bplus);
	flux_Uminus_grad_Bminus_Bminus = Prod_by_spheres_nlinV(Uminus, Bminus);
	...
	Nlin_incompress::Compute_nlin(B, Giver);
	flux_B_grad_Uplus_Bplus = Prod_by_spheres_nlinV(B, Bplus);
	flux_B_grad_Uplus_Bminus = Prod_by_spheres_nlinV(B, Bminus);

	Nlin_incompress::Compute_nlin(Bplus, Giver);
	flux_Bplus_grad_Uplus_Bplus = Prod_by_spheres_nlinV(Bplus, Bplus);
	flux_Bplus_grad_Uplus_Bminus = Prod_by_spheres_nlinV(Bplus, Bminus);

	Uminus.cvf.Copy_to(Giver.cvf);

	Nlin_incompress::Compute_nlin(B, Giver);
	flux_B_grad_Uminus_Bplus = Prod_by_spheres_nlinV(B, Bplus);
	flux_B_grad_Uminus_Bminus = Prod_by_spheres_nlinV(B, Bminus);

	Nlin_incompress::Compute_nlin(Bminus, Giver);
	flux_Bminus_grad_Uminus_Bplus = Prod_by_spheres_nlinV(Bminus, Bplus);
	flux_Bminus_grad_Uminus_Bminus = Prod_by_spheres_nlinV(Bminus, Bminus);
...
void EnergyTr::Compute_flux(FluidVF& U, FluidVF& W)
{
	...
	for (int sphere_index = 1; sphere_index <= global.energy_transfer.flux.no_spheres; sphere_index++) {	
		...
		Nlin_incompress::Compute_nlin(W, Giver);
		flux_VF_Win_Uout(sphere_index) = Prod_out_sphere_nlinV(sphere_index, W, U);	
		//  (W.graad W<). U>
	}
