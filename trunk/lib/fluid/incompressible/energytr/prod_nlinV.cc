...
//*********************************************************************************************
// Helical decomposition fluxes use it

Array<Real,1> EnergyTr::Prod_by_spheres_nlinV(FluidVF& U, FluidVF& W)
{
	Real prev_res = 0;
	Real res;
	...
	flux.resize(global.energy_transfer.flux.no_spheres+1);

	flux = 0.0;

	for (int sphere_index = 0; sphere_index < global.energy_transfer.flux.no_spheres; sphere_index++) {

		inner_radius = global.energy_transfer.flux.radii(sphere_index);
		outer_radius = global.energy_transfer.flux.radii(sphere_index+1);

		res = universal->Shell_mult_single(U.nlin1, W.cvf.V1, inner_radius, outer_radius)
			 + universal->Shell_mult_single(U.nlin2, W.cvf.V2, inner_radius, outer_radius)
			 + universal->Shell_mult_single(U.nlin3, W.cvf.V3, inner_radius, outer_radius); 

		flux(sphere_index+1) = res + flux(sphere_index);
	}
	return flux;
}
