...
void Nlin_incompress::Compute_nlin(FluidVF& U, FluidVF& W, FluidSF& T) 
{
	if (global.program.kind == "MHD_SCALAR_INCOMPRESS") 
		Compute_nlin_MHD_SCALAR_INCOMPRESS(U, W,T);
	
	// Rayleigh-Benard magnetoconvection 
	else if (global.program.kind == "MHD_SCALAR_INCOMPRESS_ALL")
		Compute_nlin_MHD_SCALAR_INCOMPRESS_ALL(U, W,T);
}


void Nlin_incompress::Compute_nlin_MHD_SCALAR_INCOMPRESS(FluidVF& U, FluidVF& W, FluidSF& T) 
{
	...
	//	fluidIO_incompress.Output_real_field(U, W, T);
	//	fluidIO_incompress.Output_field_r(U, W, T);
	//	fluidIO_incompress.Output_global_real_space(U, W, T);  // Only for Cheby basis
	//	fluidIO_incompress.Output_cout_real_space(U, W, T);	 // Only for Cheby basis
	...
	//	fluidIO_incompress.Output_field_k(U, W, T);
    //    fluidIO_incompress.Output_Tk_shell_spectrum(U, W, T);
    //    fluidIO_incompress.Output_Tk_ring_spectrum(U, W, T);
   //     fluidIO_incompress.Output_Tk_cylindrical_ring_spectrum(U, W, T);
...
void Nlin_incompress::Compute_nlin_MHD_SCALAR_INCOMPRESS_ALL(FluidVF& U, FluidVF& W, FluidSF& T) 
{
