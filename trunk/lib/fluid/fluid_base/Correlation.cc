...
Array<Real,1> Correlation::shell_kn;
Array<Real,1> Correlation::shell_knf;
...
{
	shell_kn.resize(global.spectrum.shell.no_shells);
	shell_knf.resize(global.spectrum.shell.no_shells);
...
Real Correlation::Get_energy_injection_rate(FluidVF &U)
{
	return 2*(universal->Get_total_energy(U.Force1, U.cvf.V1)
			+ universal->Get_total_energy(U.Force2, U.cvf.V2)
			+ universal->Get_total_energy(U.Force3, U.cvf.V3));
}

Real Correlation::Get_injection_rate(FluidVF &U, FluidVF &W)
{
	return 2*(universal->Get_total_energy(U.Force1, W.cvf.V1)
			+ universal->Get_total_energy(U.Force2, W.cvf.V2)
			+ universal->Get_total_energy(U.Force3, W.cvf.V3));
}
...
void Correlation::Compute_shell_spectrum(FluidVF& U)
{
	...
	universal->Compute_shell_spectrum(U.cvf.V1, 0, shell_ek1, shell_kn);
	
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum(U.cvf.V2, 0, shell_ek2, shell_kn);
	
	universal->Compute_shell_spectrum(U.cvf.V3, 0, shell_ek3, shell_kn);
	
	
	universal->Compute_shell_spectrum_force(U.cvf.V1, 2, shell_dissk1, shell_knf);
	
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum_force(U.cvf.V2, 2, shell_dissk2, shell_knf);
	
	universal->Compute_shell_spectrum_force(U.cvf.V3, 2, shell_dissk3, shell_knf);
...
{
	universal->Compute_shell_spectrum(U.cvf.V1, W.cvf.V1, 0, shell_ek1, shell_kn);
	
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum(U.cvf.V2, W.cvf.V2, 0, shell_ek2, shell_kn);
	
	universal->Compute_shell_spectrum(U.cvf.V3, W.cvf.V3, 0, shell_ek3, shell_kn);
}

void Correlation::Compute_shell_spectrum_dissk(FluidVF& U, FluidVF& W)
{
	universal->Compute_shell_spectrum_force(U.cvf.V1, W.cvf.V1, 2, shell_ek1, shell_knf);
	
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum_force(U.cvf.V2, W.cvf.V2, 2, shell_ek2, shell_knf);
	
	universal->Compute_shell_spectrum_force(U.cvf.V3, W.cvf.V3, 2, shell_ek3, shell_knf);

	shell_ek1 *= TWO * (U.dissipation_coefficient + W.dissipation_coefficient);
	shell_ek2 *= TWO * (U.dissipation_coefficient + W.dissipation_coefficient);
	shell_ek3 *= TWO * (U.dissipation_coefficient + W.dissipation_coefficient);
}

void Correlation::Compute_shell_spectrum_dissk_2nu(FluidVF& U, FluidVF& W)
{
	universal->Compute_shell_spectrum_force(U.cvf.V1, W.cvf.V1, 2, shell_ek1, shell_knf);
	...
	shell_ek1 *= TWO * U.dissipation_coefficient;
	shell_ek2 *= TWO * U.dissipation_coefficient;
	shell_ek3 *= TWO * U.dissipation_coefficient;
}
...
void Correlation::Compute_shell_spectrum(FluidVF& U, FluidSF& T)
{
	...
	universal->Compute_shell_spectrum(U.cvf.V1, T.csf.F, 0, shell_ek1, shell_kn);
    
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum(U.cvf.V2, T.csf.F, 0, shell_ek2, shell_kn);
    
	universal->Compute_shell_spectrum(U.cvf.V3, T.csf.F, 0, shell_ek3, shell_kn);
}
...
void Correlation::Compute_shell_spectrum_helicity_MHD(FluidVF& U) {
	universal->Compute_shell_spectrum_helicity_MHD(U.cvf.V1, U.cvf.V2, U.cvf.V3, shell_ek1, shell_ek2, shell_ek3);
	shell_ek1 *= TWO;
	shell_ek2 *= TWO;
	shell_ek3 *= TWO;
}
...
{
	universal->Compute_shell_spectrum(T.csf.F, 0, shell_ek, shell_kn);
	
	universal->Compute_shell_spectrum(T.csf.F, 2, shell_dissk, shell_kn);
...
{
	universal->Compute_shell_spectrum(T1.csf.F, T2.csf.F, 0, shell_ek, shell_kn);
}
...
{
	universal->Compute_shell_spectrum_force(U.cvf.V1, U.Force1, 0, shell_ek1, shell_knf);
	
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum_force(U.cvf.V2, U.Force2, 0, shell_ek2, shell_knf);
	
	universal->Compute_shell_spectrum_force(U.cvf.V3, U.Force3, 0, shell_ek3, shell_knf);
...
void Correlation::Compute_force_shell_spectrum(FluidVF& U, FluidVF& W)
{
	universal->Compute_shell_spectrum_force(W.cvf.V1, U.Force1, 0, shell_ek1, shell_knf);
	
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum_force(W.cvf.V2, U.Force2, 0, shell_ek2, shell_knf);
	
	universal->Compute_shell_spectrum_force(W.cvf.V3, U.Force3, 0, shell_ek3, shell_knf);
...
{
	universal->Compute_shell_spectrum(T.csf.F, T.Force, 0, shell_ek, shell_knf);
...
void Correlation::Compute_Tk_shell_spectrum(FluidVF& U)
{
	...
	universal->Compute_shell_spectrum(U.cvf.V1, U.nlin1, 0, shell_ek1, shell_kn);
    
	if (!global.program.two_dimension)
		universal->Compute_shell_spectrum(U.cvf.V2, U.nlin2, 0, shell_ek2, shell_kn);
    
	universal->Compute_shell_spectrum(U.cvf.V3, U.nlin3, 0, shell_ek3, shell_kn);
...
{
	universal->Compute_shell_spectrum(T.csf.F, T.nlin, 0, shell_ek, shell_kn);
