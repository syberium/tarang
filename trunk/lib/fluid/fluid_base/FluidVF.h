...
public:
	...
	void Make_incomplete_shells_zero();

	void Filter_field_and_write_real(Real k0, int dir, int type, Real kf, string field_name);
	void Filter_field_and_write_real(Real k0, int dir, int type, Real kf, string field_name, CVF complexF);

	void Helical_decomposition(FluidVF& plus, FluidVF& minus);

	void Write_real_field(string field_name);

	Real Get_dt();
	Real Get_dt(FluidSF& T);
	Real Get_dt(FluidVF& W);
	Real Get_dt(FluidVF& W, FluidSF& T);
	Real Get_dt_new(FluidVF& W);
};
